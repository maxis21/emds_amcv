            ...
            <table id="dataTable" class="table-content display">
                ...
                <tbody>
                    ...
                    <tr style="background-color: {{$color}}">
                        @if ($collection->isActive)
                        <td style="width: 25px">
                            <p id="idActive{{ $collection->id }}" class="rounded-pill bg-success" style="text-align: center; padding: 5px"> Active </p>
                        </td>
                        @else
                        <td style="width: 25px">
                            <p id="idActive{{ $collection->id }}" class="rounded-pill" style="text-align: center; padding: 5px; background-color: rgb(180, 180, 180); color:    #494949">
                                Disabled </p>
                        </td>
                        @endif
                            ...
                            <div class="dropdown">
                                ...
                                <div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
                                    <ul>
                                        ...
                                        <a onclick="resetModal('{{ $collection->id }}')">
                                            <li>Reset password</li>
                                        </a>
                                        <form action="{{ route('to.Set') }}" method="POST" id="formUser{{ $collection->id }}">
                                            @csrf
                                            <input type="hidden" value="{{ $collection->id }}" name="id">
                                            <input type="hidden" value="{{ $collection->isActive == 1 ? 0 : 1 }}" name="status">
                                        </form>
